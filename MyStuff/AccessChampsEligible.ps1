...
$BirthdaySplat = @{name = "Birthday"; expression = `
    {(get-date -uformat "%m-%d-%y" -date ("{0}-{1}-{2}" -f ($_.birthday).substring(0, 4), $_.birthday.substring(4, 2), $_.birthday.substring(6, 2)))}
}
...
Sort-Object -property G, Age, birthday, LastName, FirstName,Meet
...
$AllSwimmersCount = $AllSwimmers.count
$AllSwimmers | ForEach-Object `
    -begin {
    ...
    -process { `
        ...
        if ($_.Swimmer -eq $thisSwimmer) {
            ...
            $temp = $_.Event.replace("r", "")
            $temp = $temp.substring(0, $temp.indexof("-"))
            $thisEvents += "'" + $temp + "'"
        } #end  if same swimmer
        #Look for different swimmer or the last object in this collection.
        if ($_.Swimmer -ne $thisSwimmer -or $LoopCount -eq $AllSwimmersCount) {
            if ($MeetCount -ge 2 ) {
                $EligibleCount++
