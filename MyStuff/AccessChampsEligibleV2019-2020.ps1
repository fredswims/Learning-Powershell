...
# The next two variables change every year.
$Season="2019" #uses fall year; 2019-2020 enter as 2019
# Update this variable with all the names of the dual meets for this season.
$MeetArray="2019-12-15 Long Beach","2020-01-12 Great Neck","2020-01-26 Syosset","2020-02-09 Planview"

# These are lanes used in our dual meets. 
$LaneNames="Lane 1", "Lane 2", "Lane 3", "Lane 4", "Lane 5", "Lane 6" 

Set-Location $env:onedrive\swimclub\$season
...
#Don't include overall relay time event
    # For relay events a placeholder records has the 'Swimmer name' set to the Lane. 
    # So we must exclude Swimmer names with lane names.
    ($_.swimmer.substring(0, 6) -notin ($LaneNames) ) -and `
        ...
        if ($_.Swimmer -ne $thisSwimmer -or $LoopCount -eq $InputxCount) {
            if ($MeetCount -ge 2 ) {
                ...
                for ($index=0; $index -lt $thisEvents.count; $index++) {
                    ...
                    #don't do check if the lane was not entered - heat will be ['].
...
$ExcelPath = join-path $pwd -childpath "ChampsRoster.xlsx"
