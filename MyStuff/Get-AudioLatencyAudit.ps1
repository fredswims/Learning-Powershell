function Get-THEAudioLatencyAudit {
    ...
    Write-Host "`n🎧 Playback Device:" -ForegroundColor Yellow
    $playback | Select-Object Name, ProductName, Manufacturer, Status | Format-List

    Write-Host "`n🎙️ Recording Device:" -ForegroundColor Yellow
    ...
    Write-Host "`n🔊 Mic Monitoring Enabled:" -ForegroundColor Yellow
    if ($listenEnabled) {
        Write-Host "✅ 'Listen to this device' is enabled." -ForegroundColor Green
    } else {
        Write-Host "❌ Mic monitoring is disabled." -ForegroundColor Red
    }
    ...
    Write-Host "`n🧪 Audio Enhancements:" -ForegroundColor Yellow
    if ($enhancementsDisabled) {
        Write-Host "✅ Enhancements are disabled." -ForegroundColor Green
    } else {
        Write-Host "⚠️ Enhancements may be active—could add latency." -ForegroundColor DarkYellow
    }

    Write-Host "`n📦 Driver Info:" -ForegroundColor Yellow
