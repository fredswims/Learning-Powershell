...
Const cRowOrig = 1      'Row where the table begins.
Const cColOrig = 1      'Column where the table begins.
Const cLastLine = "Net" 'Row where the table ends.
...
sEventFilePath = ThisWorkbook.Path & cEventFilePath
    ...
    f.Close
    MsgBox ("MakeEvents: Created " & iRecordCount & " events. Last run " & Cells(cRowOrig, cColOrig + 6).Value & ". Finis!")
    Cells(cRowOrig, cColOrig + 6).Value = iRecordCount & " Events"
