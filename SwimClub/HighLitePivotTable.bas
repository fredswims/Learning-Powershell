...
'Added cCompact 2019-10-09 - Compacts both Pivot Tables
Const cCompact = vbTrue
Const cPVNAME = "ptBestTimesAllMeets"
Const cPVShortName = "ptShort"

Const cBoysSheetName = "Boys"
Const cGirlsSheetName = "Girls"
...
'Rev 9.0 2019-10-10
    'New Const cCompact = vbTrue for makeing compact PV tables.
    'Changes in this subroutine to support compact mode.
    'Changes in Hide-Times button; renamed second pivot table column name from 'Best Time' to 'Best-Time'.
    'Made changes to accomodate printing pivot tables as as html for use on Web page. Auto printing not yet in place.
    'Hard column references replaced by code such as
    'Worksheets(sSheetName).Activate
    'ActiveSheet.Columns(pt.RowRange.Column + 2).EntireColumn.Delete
'Rev 9.1 '2019-10-17 FAJ
    'Displaying both Division Record Holder and added Club Record Holder.
    'Adjustments had to be made to the iEventRank counter.
    'Added msgbox to end 'the run' after the pivot tables are deleted.
    'AppendMeetToPV had to fix how we scrape data off of the large pivot table.
'Rev 9.2 2019-10-19 FAJ
    'Created SavePivotTableAsHtml() to print compact pivot tables.
    'Now callable from Powershell.
...
sStartTime = "Began at " & Date & " " & Time()
Debug.Print cThisSub & sStartTime

    sWorkbookName = ThisWorkbook.Name
    sSheetName = ActiveSheet.Name
    Debug.Print cThisSub & " This workbook is  " & ThisWorkbook.Name
    Debug.Print cThisSub & " This sheet it " & ActiveSheet.Name
    
    If Not (ActiveSheet.Name = cBoysSheetName Or ActiveSheet.Name = cGirlsSheetName) Then
        MsgBox "Active Sheet is neither Boys or Girls - stopping program", vbCritical, cThisSub
        End 'stops execution
    End If
...
StepThruLongPivotTable:
    ...
    Else
       ...
       're-initialize array of events and times for next iteration.
        ...
        'Have we moved into a new event?
        If (sLastEvent = sThisEvent) Then
            'not the first name in this event.
            If Mid(Cells(iThisNameRow, iNameCol).Text, 1, 1) = "*" Then
                '2019-10-17 FAJ
                'Record holders do not increment the rank counter.
                'We have a record holder; don't increment the rank
                'We can have multiple record holders and
                'during the season the record holder(s) may move down the list
                'iEventRank = iEventRank 'don't increment rank.
            Else
        ...
        Else
            ...
            If Mid(Cells(iThisNameRow, iNameCol).Text, 1, 1) = "*" Then
                ...
                'iEventRank = 0
            Else
                iEventRank = 1 'what we want the top rank to be. Alternative iEventRank=iEventRank+1
            End If
        End If
...
AutoSizeDeltaTimeColumn:
    Columns(iTimeCol + 1).EntireColumn.AutoFit
...
MakePivotTableCompact:
    Worksheets(sSheetName).Activate 'get back to the sheet with the pivot tables.
    For Each pt In ActiveSheet.PivotTables
        If cCompact Then
            ActiveSheet.PivotTables(pt.Name).PivotFields("Event").LayoutCompactRow = vbTrue 'looks better on web page.
          'The compact operation shifted data to the left and left a blank column between the pivot table and the meet location.
          'Delete the blank column, autofit the meet location column and insert a column after the meet location column.
          If pt.Name = cPVShortName Then
...
GetSizeOfPivotTables:
'Get region of each Pivot Table - may use to print them.
    For Each pt In ActiveSheet.PivotTables
            iRowOrigin = pt.RowRange.Row
            iColOrigin = pt.RowRange.Column
            iLastRowInTable = iRowOrigin + (pt.RowRange.Rows.count - 1) 'last row number.
            Debug.Print cThisSub & "Pivot Table: " & pt.Name & " Rows " & iRowOrigin & ":" & iLastRowInTable & " Columm: " & iColOrigin
            Debug.Print cThisSub & "start column of Pivot Table " & pt.Name & " is " & iColOrigin & " Last row is " & iLastRowInTable
    Next
...
HiliteSubEnd:
'stub to print small pivot table
'    Range("L1:O363").Select
'    With ActiveWorkbook.PublishObjects.Add(xlSourceRange, _
'        "C:\Users\freds_000\OneDrive\SwimClub\2019\reports\AtestHTMLfile2.html", ActiveSheet.Name, _
'        "$L$1:$O$363", xlHtmlStatic, "", "")
'        .Publish (False) 'this saves report
'        .AutoRepublish = False
'    End With
sTemp = "Complete"
...
'Add ability to delete pivots tables and stop.
'This will allow the insertion of events on the sheet.
'Or move the events to another sheet
'or ultimately, with Powershell gather the data from MSAccess, and use ImportExcel
For Each pt In ActiveSheet.PivotTables
...
Next
    ...
    sAnswer = MsgBox("Pivot Tables deleted; do you want to continue?", vbYesNo, cThisSub)
...
'^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    ActiveSheet.PivotTables(cPVNAME).PivotFields("Swimmer").LayoutCompactRow = False
...
For Each pt In ActiveSheet.PivotTables
  If pt.RowRange.Column = iCol Then
    ...
    'Rename column name
    ActiveSheet.PivotTables(pt.Name).PivotFields("Best Time").Caption = _
        "Best-Time"
...
'Appends swim events to Boys or Girls sheet.
'The swim events were generated from the Meet Sheets.

    Debug.Print cThisSub & " This workbook is " & ThisWorkbook.Name
    Debug.Print cThisSub & " This sheet is " & ActiveSheet.Name
    ...
    sAnswer = MsgBox("Do you want to copy the Swim Events to " & ActiveSheet.Name, vbYesNo, cThisSub)
    If (sAnswer = vbCancel) Or (sAnswer = vbNo) Then
        MsgBox "Stopping", vbCritical, cThisSub
        End
    End If
...
Private Sub AppendMeetToPV(ByVal sThisPvName, ByRef colBestMeet, ByRef iReturn As Integer)
    ...
    For iThisRow = iRowStart To iLastRowInTable
        ...
        Else
           ...
           '2019-10-17 FAJ This is a real kludge.
           If (InStr(5, sMyString, "-", vbTextCompare) = 5 And InStr(8, sMyString, "-", vbTextCompare) = 8) Then
           'the string has a date and probably looks like 2018-11-18 Uniondale
           sMyString = Mid(sMyString, InStr(1, sMyString, " ") + 1)
           Else
           'The string is for a Record Holder and can have a form like Club or 2009 Long Beach
           'Debug.Print sMyString
           End If
           'inserted "-" because the space isn't rendering correctly on the html reports.
...
Sub HideTimes()
' Hides the swim times on the small pivot table.
Dim iCol As Integer
Dim iRow As Integer
Dim iThisCol As Integer
Const cThisSub = "HideTimes: "
    'The column is hidden by changing the font size to 1.
    'Alternately we can toggle the property "Columns(x).hidden"
    'but a PDF and a HTML file show the column.
    ...
    iThisCol = FindColumnInTopRow("Best-Time")
    Columns(iThisCol).Select
    
    If Selection.Font.Size = 1 Then
        'it is hidden - unhide it by setting the font to size of adjacent cell.
        Selection.Font.Size = Columns(iThisCol + 1).Font.Size
    Else
        'it is not hidden - hide it.
        Selection.Font.Size = 1 'hide the column
    End If
...
End Sub
Sub SavePivotTableAsHtml()
'Author 2019-10-19 FAJ
'Save-As HTML the short pivot table
...
sNames(0) = cBoysSheetName
sNames(1) = cGirlsSheetName

For Each vName In sNames()
    Worksheets(vName).Activate
    For Each pt In ActiveSheet.PivotTables
        If cCompact Then
            If pt.Name = cPVShortName Then
